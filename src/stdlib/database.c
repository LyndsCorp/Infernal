 ...
 * ============================================================
 * Los builtins de archivo interpretan las rutas relativas contra el
 * directorio del script que se está ejecutando (current_source_file),
 * no contra el CWD. Así updater/verificar_existencia.inf puede usar
 * "../../database/..." y encontrar el archivo respecto a su propia
 * ubicación, independientemente de dónde se haya lanzado infernal.
 *
 * Si la ruta es absoluta se devuelve una copia tal cual.
 * Si no se puede resolver (current_source_file es NULL o realpath
 * falla) se devuelve una copia de la original para no romper el
...
static Value builtin_tofile(int argc, Value *args) {
    ...
    if (!text) {
        free(resolved);
        error(current_eval_line, "Error al serializar el valor a texto");
    }
...
static Value builtin_fromfile(int argc, Value *args) {
    ...
    size_t data_len;
    char *data = read_file(resolved, &data_len);
    free(resolved);
...
static Value builtin_tobinfile(int argc, Value *args) {
    ...
    if (!serialized) {
        free(resolved);
        error(current_eval_line, "Error al serializar el valor a binario");
    }
...
static Value builtin_frombinfile(int argc, Value *args) {
    ...
    size_t data_len;
    unsigned char *data = (unsigned char*)read_file(resolved, &data_len);
    free(resolved);
